...
PREVIOUS_DASHBOARD_HOST="${DASHBOARD_HOST:-}"
PREVIOUS_CONTENT_HOST="${CONTENT_HOST:-}"
...
# Preserve operator overrides while updating the fields bootstrap owns. The
# complete candidate is validated before atomic replacement; .previous retains
# the old file with its ownership and mode. No credential is passed in argv.
if [[ -z "${DASHBOARD_ORIGIN:-}" || "$DASHBOARD_ORIGIN" == "https://$PREVIOUS_DASHBOARD_HOST" ]]; then
  DASHBOARD_ORIGIN="https://$DASHBOARD_HOST"
fi
if [[ -z "${CONTENT_ORIGIN:-}" || "$CONTENT_ORIGIN" == "https://$PREVIOUS_CONTENT_HOST" ]]; then
...
export PORT DASHBOARD_HOST CONTENT_HOST DASHBOARD_ORIGIN CONTENT_ORIGIN
export AUTH_SIGNING_PUBKEY AUTH_COOKIE_NAME AUTH_LOGIN_URL ADMIN_EMAIL_DOMAIN
export PAGE_COOKIE_SECRET RAW_TOKEN_SECRET API_TOKEN_PEPPER DATABASE_URL
node "$APP_DIR/scripts/write-env.js" "$ENV_FILE" || die "environment generation failed — previous file retained"
chmod 0640 "$ENV_FILE"
