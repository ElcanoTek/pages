...
//
//   node scripts/token.js add <label> [scope]   # scope: deploy (default) | admin
...
async function main() {
  ...
  switch (cmd) {
    case "add": {
      ...
      if (!label) throw new Error("usage: token.js add <label> [scope]");
      const t = await tokens.mint({ label, scope });
    ...
    case "list": {
      ...
      for (const r of rows) {
        ...
        console.log(`#${r.id}  ${r.prefix}…  ${r.label}  [${r.scope}]  ${state}  ${used}`);
      }
      break;
    }
    case "revoke": {
    ...
    default:
      console.log("usage: token.js add <label> [scope] | list | revoke <id>");
      process.exitCode = 1;
  }
}
