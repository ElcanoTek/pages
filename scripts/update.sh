...
# Register the templates this checkout ships (docs/TEMPLATES.md). Runs AFTER the
# health check, against the swapped-in app, and deliberately does not `die`:
# a template is design data, so a broken one must not fail a deploy that has
# already proven the service healthy — it reports and leaves the previous
# revision current. Identical bytes are a no-op, so re-running is free, and
# nothing already deployed moves: every page stays pinned to the revision it was
# built from. Set PAGES_SKIP_TEMPLATE_SYNC=1 to skip.
if [[ "${PAGES_SKIP_TEMPLATE_SYNC:-0}" != "1" ]] && [[ -f "$APP_DIR/scripts/template.js" ]] && [[ -d "$APP_DIR/templates" ]]; then
  step "Syncing shipped templates"
  if [[ -f "$ENV_FILE" ]] && grep -q '^DATABASE_URL=' "$ENV_FILE"; then
    runuser -u "$APP_USER" -- bash -c "set -a; . '$ENV_FILE'; set +a; cd '$APP_DIR' && node scripts/template.js sync" \
      || warn "template sync reported problems — the previous revisions stay current"
  else
    warn "no DATABASE_URL in $ENV_FILE — skipping template sync"
  fi
fi
