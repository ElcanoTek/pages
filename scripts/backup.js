...
async function run(command, args, { env = process.env, cleanup = false } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      ...
      reject(new Error(`${path.basename(command)} failed (${code}): ${err.trim()}`));
    });
  });
}
...
function isolatedEnv(socket) {
  ...
  return { ...env, PGHOST: socket, PGPORT: "5432", PGUSER: "pages_backup", PGDATABASE: "pages",
    NODE_ENV: "production", RAW_TOKEN_SECRET: crypto.randomUUID(), API_TOKEN_PEPPER: crypto.randomUUID(),
    PAGE_COOKIE_SECRET: crypto.randomUUID(), PAGES_DEV_LOGIN: "0", AUTH_SIGNING_PUBKEY: "",
...
async function rehearse(directory) {
  ...
  const scratch = await fs.mkdtemp(path.join(os.tmpdir(), "pgb-"));
  ...
  const asPg = (tool, args, options) => process.getuid?.() === 0
    ? run("runuser", ["-u", "postgres", "--", tool, ...args], options) : run(tool, args, options);
  try {
    ...
    await asPg(pgctl, ["-D", data, "-l", path.join(scratch, "postgres.log"), "-w", "-t", "60", "-o",
      `-k ${socket} -c listen_addresses='' -c unix_socket_permissions=0700`, "start"]);
    started = true;
    const env = isolatedEnv(socket);
    ...
    if (started) await asPg(pgctl, ["-D", data, "-w", "-m", "immediate", "stop"], { cleanup: true });
...
async function create(destination, application, environment, installConfig) {
  ...
  try {
    ...
    await run(pgdump, ["--format=custom", "--no-owner", "--no-acl", "--file=" + path.join(temporary, "database.dump")], { env: dumpEnv });
    ...
    for (const [name, source] of [["install.env", installConfig], ["local.env", path.join(application, ".env")]]) {
      try { await fs.copyFile(source, path.join(temporary, "config", name)); optional[name] = true; }
    ...
    const restored = await rehearse(temporary);
    const manifest = { format_version: 1, status: "complete", started_at: startedAt, completed_at: new Date().toISOString(),
      node_version: process.version, pg_dump_version: await run(pgdump, ["--version"]), optional_configuration: optional,
...
async function check(directory) {
  ...
  const restored = await rehearse(directory);
...
module.exports = { create, check, verifyFiles, inventory, isolatedEnv };
