...
# only), runs migrations, ensures an agent API token exists, and
# boots the server with both vhosts pointed at localhost so you can drive the
# REST API + /raw render loop locally — the "let an agent deploy pages" loop.
# Idempotent: re-running reuses the cluster, secrets, and token.
#
