...
async function verify(rawToken) {
  ...
  const t = await loadActiveToken("t.token_hash = $1", hashToken(rawToken));
  if (!t) return null;
...
// verifyId — the same live lookup by row id, for a credential Pages minted on a
// token's behalf (lib/data-export.js). Revoking the token, or a grant no longer
// holding the page, must end what it minted exactly as it ends the token.
// Does not touch last_used_at: nothing presented the token itself.
async function verifyId(tokenId) {
  if (!/^[1-9][0-9]*$/.test(String(tokenId))) return null;
  return loadActiveToken("t.id = $1", String(tokenId));
}

// `predicate` is one of the two literals above, never caller input.
async function loadActiveToken(predicate, value) {
  const { rows } = await db.query(
    `SELECT t.id, t.label, t.scope,
      ...
      WHERE ${predicate} AND t.revoked_at IS NULL
