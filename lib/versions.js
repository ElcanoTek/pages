...
// setPassword — set or clear the per-page client password (PLAN §6b). Empty /
// null clears it (page becomes Elcano-only). Allowed for agents (bearer) and
// admins; the hash is scrypt (lib/pagecookie).
async function setPassword({ slug, password }, actorCtx) {
  return db.withTransaction(async (client) => {
    const page = await lockPage(client, slug);
    const hash = password === null || password === undefined || password === "" ? null : pagecookie.hashPassword(password);
