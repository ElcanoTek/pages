...
const db = require("./db");
const { RESERVED_SLUG_SEGMENTS, assertSlugNotReserved } = require("./route-namespace");
...
function freshnessOf(row, now = Date.now()) {
  ...
  // Keep explicit check history, but never attach its failure to a later
  // successful live refresh. On equal timestamps the explicit check wins.
  const refreshIsLatest = refreshedAt && (!lastCheckAt || Date.parse(refreshedAt) > Date.parse(lastCheckAt));
  return {
    last_check_at: lastCheckAt,
    latest_outcome: refreshIsLatest ? "updated" : (row && row.last_check_outcome) || null,
    latest_detail: refreshIsLatest ? null : (row && row.last_check_detail) || null,
    latest_source_as_of: refreshIsLatest ? sourceAsOf : iso(row && row.last_check_source_as_of),
...
// Legacy full-source versions predate indexed managed metadata. Read their
// validated envelope without rewriting immutable history. New writes always
// populate the index; unrelated plain HTML and malformed historical contracts
// contribute no invented data timestamps.
function legacyManagedMetadata(html) {
  if (!html) return null;
  try { return pageData.parseManagedHtml(html); }
  catch (error) { if (!(error instanceof ApiError)) throw error; return null; }
}

function indexedOrLegacyCoverage(row) {
  const managed = legacyManagedMetadata(row.legacy_managed_html);
  return {
    source_as_of: row.source_as_of || (managed && managed.envelope.source_as_of),
    refreshed_at: row.refreshed_at || (managed && managed.envelope.refreshed_at),
  };
}

async function sourceCoverageFloor(client, pageId, currentSource = null) {
  const { rows } = await client.query(
    `SELECT max(source_as_of) AS source_as_of, NULL::text AS legacy_managed_html
       FROM page_versions WHERE page_id=$1 AND status <> 'rejected'
     UNION ALL
     SELECT NULL::timestamptz, html FROM page_versions
      WHERE page_id=$1 AND status <> 'rejected' AND source_as_of IS NULL
        AND data_sha256 IS NULL AND position('pages-data' IN html) > 0`,
    [pageId]
  );
  let floor = currentSource ? Date.parse(currentSource) : null;
  for (const row of rows) {
    const value = indexedOrLegacyCoverage(row).source_as_of;
    if (value) floor = Math.max(floor === null ? -Infinity : floor, new Date(value).getTime());
  }
  return floor === null ? null : new Date(floor).toISOString();
}
...
async function listPages(executor = db, options = undefined) {
  ...
  const { rows } = await executor.query(
    `SELECT p.id, p.slug, p.title, p.client_id, p.workspace_id, w.name AS workspace_name,
            ...
            CASE WHEN pv.data_sha256 IS NULL AND position('pages-data' IN pv.html) > 0
                 THEN pv.html END AS legacy_managed_html,
  ...
  const now = Date.now();
  return rows.map(
    ({ source_as_of, refreshed_at, legacy_managed_html, last_check_at, last_check_outcome, last_check_detail, last_check_source_as_of, ...page }) => ({
      ...page,
      freshness: freshnessOf(
        { ...indexedOrLegacyCoverage({ source_as_of, refreshed_at, legacy_managed_html }), last_check_at, last_check_outcome, last_check_detail, last_check_source_as_of },
        now
      ),
    })
  );
...
async function recordRefreshCheck({ slug, outcome, detail = null, sourceAsOfSeen = null, now = Date.now() }, actorCtx) {
  ...
  return db.withTransaction(async (client) => {
    ...
    const coverage = indexedOrLegacyCoverage(rows[0] || {});
    return {
      slug: page.slug,
      freshness: freshnessOf(
        {
...
async function createPage({ slug, title = "", clientId = null, workspaceId = null, requireApproval = false }, actorCtx) {
  assertBooleanValue(requireApproval, "require_approval");
  slug = normalizeSlug(slug);
...
function assertManagedBlocksDeployable(html) {
  try {
    return pageData.parseManagedHtml(html);
  } catch (error) {
    const code = error && error.code;
    if (code === "page_not_data_managed" || code === "page_not_template_managed") return null;
    if (!code) throw error;
    throw conflict(
...
function prepareDeploy(
  ...
  assertBooleanValue(publish, "publish");
  if (typeof html !== "string" || html.length === 0) throw badRequest("html is required", "html_required");
  // A Bento deck is an application that carries its own complete design, so it
  // is raw unless the caller says otherwise — and "otherwise" can only be a
  // mistake: a themed deck gets Flag's stylesheet links spliced in ahead of the
  // deck's own CSP <meta> and a Page menu drawn over its toolbar. Refuse that
  // with the fix in the message rather than storing a version that renders wrong.
  const deck = render.isBentoDeck(html);
  if (renderMode === undefined || renderMode === null) renderMode = deck ? "raw" : "themed";
  if (!RENDER_MODES.has(renderMode)) throw badRequest(`bad render_mode: ${renderMode}`, "bad_render_mode");
  if (deck && renderMode !== "raw") {
    throw badRequest(
      `this document is a Bento deck (it carries a <script type="application/bento+json"> block), which Pages serves raw: ` +
        `it is its own application with a complete design, so render_mode "themed" would splice Flag's head tags ahead of ` +
        `the deck's own CSP and draw a Page menu over its toolbar. Omit render_mode or pass "raw".`,
      "bad_render_mode"
    );
  }
  ...
  // Likewise the deck host adaptation: a file saved from the hosted editor
  // carries the tag Pages injected at render time, and that file is exactly
  // what gets uploaded back as the next version.
  const deployHtml = render.stripDeckHostAdaptation(render.stripInjectedNav(normalized.html));
  const managed = assertManagedBlocksDeployable(deployHtml);
  const dedupeByData = dataMetadata !== null;
  dataMetadata = managed ? {
    action: dataMetadata ? dataMetadata.action || "data_update" : "deploy",
    dataSha: managed.data_sha256,
    schemaSha: managed.schema_sha256,
    templateSha: managed.template_sha256,
    sourceAsOf: managed.envelope.source_as_of,
...
// Deck saves continue from the latest editor snapshot, not the live pointer.
// Use the same page lock and version state machine as every other authoring
// path, but never dedupe a deliberate edit onto an older historical version.
async function saveDeck({ slug, pageId, baseVersion, html }, actorCtx) {
  if (actorCtx.actorType !== "user") throw forbidden("a deck edit session requires a staff actor", "admin_only");
  if (typeof baseVersion !== "string" || !/^[1-9][0-9]{0,18}$/.test(baseVersion) || BigInt(baseVersion) > 9223372036854775807n) {
    throw badRequest("reopen the deck from the admin before saving: a valid X-Pages-Base-Version is required", "deck_base_version_required");
  }
  const prepared = prepareDeploy({
    slug, html, renderMode: "raw", note: "Saved from the Bento editor", source: "admin", publish: false,
  }, actorCtx);
  return db.withTransaction(async (client) => {
    const page = await lockPage(client, prepared.slug);
    if (!sameDbId(page.id, pageId) || page.disabled) {
      throw forbidden("this edit session is no longer available for this page", "deck_session_unavailable");
    }
    const latest = (await client.query(
      `SELECT id, html, content_sha256, render_mode, status FROM page_versions
        WHERE page_id = $1 ORDER BY created_at DESC, id DESC LIMIT 1`, [page.id]
    )).rows[0];
    if (!latest || !render.isBentoDeck(latest.html) || !render.isBentoDeck(prepared.html)) {
      throw forbidden("this page is no longer a deck — reopen it from the admin", "deck_session_unavailable");
    }
    const base = await client.query(`SELECT id FROM page_versions WHERE page_id = $1 AND id = $2`, [page.id, baseVersion]);
    const sameSnapshot = latest.content_sha256 === prepared.contentSha && latest.render_mode === prepared.renderMode
      && latest.status !== "rejected" && latest.status !== (page.require_approval ? "draft" : "pending");
    if (!base.rows.length || (!sameDbId(latest.id, baseVersion) && !sameSnapshot)) {
      throw conflict(
        "a newer deck version was saved — reopen the editor from the admin and reconcile your changes before saving again",
        "stale_deck_version", { base_version_id: baseVersion, latest_version_id: String(latest.id) }
      );
    }
    return deployLocked(client, page, prepared, actorCtx, { dedupeVersionId: latest.id });
  });
}
...
// Create-only callers may supply assertExistingPage. It runs with the final
// page row locked, including after an INSERT loses a concurrent create, so a
// missing-row check cannot turn a rejected replacement into an ordinary deploy.
async function createAndDeployWithClient(
  client,
  { title = "", clientId = null, requireApproval = false, ...deployArgs },
  actorCtx,
  { assertExistingPage = null } = {}
) {
  assertBooleanValue(requireApproval, "require_approval");
  ...
  if (!page) {
    ...
    if (page) {
      ...
      // index. Take the normal page lock before checking the caller's contract.
      page = (
        await client.query(
          `SELECT id, slug, require_approval, disabled, published_version_id
             FROM pages WHERE slug = $1 AND deleted_at IS NULL FOR UPDATE`,
  ...
  if (!created && assertExistingPage) await assertExistingPage(page);
  return withConfigSchemaNotice(prepared, { created, ...(await deployLocked(client, page, prepared, actorCtx)) });
}

async function deployLocked(client, page, prepared, actorCtx, { dedupeVersionId = null } = {}) {
  const {
    html,
    renderMode,
    note,
  ...
  assertExpectedVersion(page, expectedVersion);
    ...
    // Binding is immutable provenance: identical output from another design or
    // revision still needs a distinct version. A plain deploy stays detached.
    const existing = dedupeByData
      ? await client.query(
          `SELECT id, page_id, content_sha256, status, render_mode, author, source,
            ...
            WHERE page_id = $1
              ...
              AND template_version_id IS NOT DISTINCT FROM $7::bigint
              AND status <> 'rejected'
              ${page.require_approval ? "AND status <> 'draft'" : "AND status <> 'pending'"}
            ORDER BY id DESC LIMIT 1`,
          [page.id, dataMetadata.dataSha, dataMetadata.templateSha, dataMetadata.sourceAsOf, renderMode, dedupeVersionId, templateBinding ? templateBinding.templateVersionId : null]
        )
      : await client.query(
          `SELECT id, page_id, content_sha256, status, render_mode, author, source,
                  note, reviewed_by, reviewed_at, created_at
            ...
            WHERE page_id = $1 AND content_sha256 = $2
              ...
              AND template_version_id IS NOT DISTINCT FROM $5::bigint
              AND status <> 'rejected'
              ${page.require_approval ? "AND status <> 'draft'" : "AND status <> 'pending'"}
            ORDER BY id DESC LIMIT 1`,
          [page.id, contentSha, renderMode, dedupeVersionId, templateBinding ? templateBinding.templateVersionId : null]
        );
...
async function updatePageDataWithClient(
...

  {
    ...
    const sourceFloor = await sourceCoverageFloor(client, page.id, managedTemplate.envelope.source_as_of);
...
async function setDisabled({ slug, disabled }, actorCtx) {
  assertAdminAction(actorCtx);
  assertBooleanValue(disabled, "disabled");
  return db.withTransaction(async (client) => {
    const page = await lockPage(client, slug);
    ...
    return { slug: page.slug, disabled };
  });
}
...
function assertBooleanValue(value, field) {
  if (typeof value !== "boolean") throw badRequest(`${field} must be a boolean`, "bad_boolean");
}
