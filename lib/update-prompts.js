...
function managedPrompt({ slug, instructions, schemaSha256, publish, recurring, sources = null }) {
  ...
  return [
    ...
    `9. Build one complete data object satisfying the returned JSON Schema. Recompute source-derived values from complete source coverage; never invent zeros, silently drop rows, average ratios, or publish a partial object. ${FABRICATED_ROWS_CLAUSE}`,
    ...
    "11. Set source_as_of to the latest source coverage actually represented. Decide the transport in step 10 BEFORE the audit, then call confirm_audit once for the single managed-data write, naming the tool you will actually call (mcp_pages_update_page_data_upload or mcp_pages_update_page_data), then make that one write with the returned live_version_id as expected_version and PUBLISH exactly as specified. If the declared inline call is rejected for SIZE and nothing has executed, retire that declaration with confirm_audit(success=false, ...) and call confirm_audit again naming mcp_pages_update_page_data_upload before publishing — a publish through an undeclared tool fails verification even when the data landed. Any other rejection (stale_version, schema, reconciliation) keeps the declared tool and follows its own retry rule.",
