...
const CONTRACT_FILES_CLAUSE =
  "Then fetch the schema and complete live envelope ONCE into workspace files: call mcp_pages_get_page_data with detail=\"export\" and download its schema_url and envelope_url (data_url too if useful) with a host-side URL download tool such as download_url, checking sha256 of the schema file against schema_sha256 and of the data file against data_sha256; only if your client cannot download URLs, make one detail=\"full\" read instead. " +
...
function managedPrompt({ slug, instructions, schemaSha256, publish, recurring, sources = null }) {
  ...
  return [
    ...
    "11. On stale_version or an ambiguous write response, reread once with detail=\"summary\". If its data_sha256 and coverage already match your payload, verify that version and finish; otherwise refresh the contract files once as in step 1, reconcile against the new live version, and retry once. Never reuse stale hashes or expected_version.",
