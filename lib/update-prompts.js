...
const { measureData } = require("./data-limits");
...
// commitTool pins the update branch's single Pages write (recurring prompts
// choose it from the live payload size at preparation); without it both commit
// paths stay available, as a one-time prompt still lets the run choose.
const INLINE_COMMIT_TOOL = "mcp_pages_update_page_data";
const UPLOAD_COMMIT_TOOL = "mcp_pages_update_page_data_upload";
const UPLOAD_TRANSPORT_TOOLS = ["mcp_pages_start_page_upload", "mcp_pages_append_page_upload"];
// The prompt's long-standing transport line: above this, a model must not emit
// the payload as one inline argument.
const INLINE_DATA_MAX_BYTES = 20000;

// commitToolFor — decided once, at preparation, from what the page holds now.
// Unknown size stages: the upload path works at any size, inline does not.
function commitToolFor(liveDataBytes) {
  return Number.isSafeInteger(liveDataBytes) && liveDataBytes <= INLINE_DATA_MAX_BYTES
    ? INLINE_COMMIT_TOOL
    : UPLOAD_COMMIT_TOOL;
}

function executionRequirements(sources, mode, { slug = null, commitTool = null } = {}) {
  const managed = mode === "managed_data";
  const commitTools = commitTool ? [commitTool] : [INLINE_COMMIT_TOOL, UPLOAD_COMMIT_TOOL];
  ...
  const tools = new Set(managed ? [
    ...
    "mcp_pages_preflight_page", ...commitTools,
    ...(commitTools.includes(UPLOAD_COMMIT_TOOL) ? UPLOAD_TRANSPORT_TOOLS : []),
  ] : []);
  for (const source of sources || []) {
  ...
  return {
    ...
    ...(managed ? {
      ...
      completion: { any_succeeded: [...commitTools, "mcp_pages_record_refresh_check"].sort() },
...
// Truncated rows cannot prove preservation; source reconciliation and the full
// live payload (when available) have separate roles.
const TRUNCATED_ENVELOPE_CLAUSE =
  "If your runtime truncates that response, recover the complete schema and metadata through a supported file or narrower read. Never reconstruct data or registries from a preview. Rebuild the complete object from complete source coverage; request include_data=true for historical overlap when your client can retrieve the full live payload. If unavailable, report that limitation: totals alone do not prove row-level equality.";
...
// The update branch's write, stated rather than left to the run. A model that
// picked its transport at step 9 declared one commit tool to the audit and then
// called the other. commitTool is null for a one-time prompt, which keeps the
// size rule a human-supervised run has always followed.
function commitSteps(commitTool) {
  if (!commitTool) {
    return [
  ...
  const common = "Never claim a tool rejected data unless an actual call returned that error. A transient failure allows one retry of the same exact bytes; stop on an explicit policy, authentication or DNS failure.";
  const commit =
    `10. Commit with ${commitTool} — the COMMIT TOOL, and this branch's only Pages write — using this run's live_version_id as expected_version, source_as_of equal to the latest source date actually represented, the reconciled expect, and PUBLISH as specified. ` +
    `Follow the caller's configured approval workflow before the gated mutation, if one is required, and declare exactly ${commitTool} as the update-branch mutation; never declare or call another commit tool.`;
  if (commitTool === UPLOAD_COMMIT_TOOL) {
    return [
      "9. Stage the complete data file for the COMMIT TOOL: " + fileUploadGuidance("data", "mcp_pages_") +
        ` Never paste the file into an inline argument. If no permitted file transport works, select blocked and keep the file. ${common}`,
      commit,
    ];
  }
  return [
    `9. Send the complete object inline to the COMMIT TOOL; this page's payload is small enough that staging is unnecessary. Do not stage or upload it. If ${commitTool} refuses it as too large for inline transport, select blocked, keep the file, and report that this prompt must be prepared again. ${common}`,
    commit,
  ];
}

function managedPrompt({ slug, instructions, schemaSha256, publish, recurring, sources = null, commitTool = null }) {
  ...
  return [
    ...
    ...(commitTool ? [`COMMIT TOOL: ${commitTool}`] : []),
    "EXECUTION REQUIREMENTS (JSON):",
    JSON.stringify(executionRequirements(sources, "managed_data", { slug, commitTool })),
    commitTool === INLINE_COMMIT_TOOL
      ? "The scheduler must supply these capabilities. This page commits inline with the COMMIT TOOL, so no upload transport is needed."
    ...
    "READ AND CHECK",
    `1. Call mcp_pages_get_page_data for exactly ${slug} with include_data=false to read the contract without a large payload. Never create another page, companion data page, or replacement slug. Read live_version_id, schema, schema_sha256, template_sha256, envelope.source_as_of, data_profile and dimension values. ${TRUNCATED_ENVELOPE_CLAUSE}`,
    recurring
      ? "2. Stop as blocked if disabled, without a live managed version, or require_approval=true. Use this run's schema and hashes as the contract. Read current CONFIG deal/KPI registries with mcp_pages_get_page_config when the page uses them. Preserve stable identifiers and metric meaning. A changed hash alone is not failure; incompatible grain, required fields, source identity or mappings are blocked, never an unattended schema migration."
    ...
    "8. Reconcile source-to-payload row count, date range, dimensions, unmapped counts and every numeric total. Aggregate numerators and denominators before ratios. Validate with a real JSON Schema validator or Pages validation; manual spot checks are not full schema validation. Write one complete JSON file and save reconciliation evidence separately. Pass source-computed expect counts, ranges and numeric totals supported by the data_profile; verify any unprofiled tuple fields locally.",
    ...commitSteps(commitTool),
    "11. On stale_version or an ambiguous write response, reread once. If the intended data and coverage are already live, verify that version and finish; otherwise retry once only after checking the current contract and reconciling against the new live version. Never reuse stale hashes or expected_version.",
...
async function prepare({
  ...
  if ((recurring || updateType === "data") && !managed) {
    ...
    mode = "managed_data";
    // A recurring run happens unattended; its one Pages write is decided here,
    // from the payload the page holds now, not by the model at step 9.
    if (recurring) commitTool = commitToolFor(measureData(managed.envelope.data).payload_bytes);
  ...
  return {
    ...
    execution_requirements: executionRequirements(sources, mode, { slug, commitTool }),
