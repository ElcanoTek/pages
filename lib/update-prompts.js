...
function executionRequirements(sources, mode) {
  ...
  const tools = new Set(mode === "managed_data" ? [
    ...
    "mcp_pages_update_page_data_upload", "mcp_pages_preflight_page",
  ] : []);
  for (const source of sources || []) {
...
function managedPrompt({ slug, instructions, schemaSha256, publish, recurring, sources = null }) {
  ...
  return [
    ...
    "The scheduler must supply these common capabilities and at least one permitted upload transport. Direct file HTTPS is preferred; ordered MCP chunks work without outbound file HTTP. Verify the chosen transport tools before processing.",
    ...
    "9. Over 20,000 UTF-8 bytes, stage the complete data file. " + fileUploadGuidance("data", "mcp_pages_") + " A smaller object may use mcp_pages_update_page_data inline. Never paste a large file into one inline argument. If no permitted file transport works, select blocked and keep the file. A transient upload failure allows one retry of the same exact bytes; stop on an explicit policy, authentication or DNS failure.",
