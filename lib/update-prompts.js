...
function executionRequirements(sources, mode) {
  ...
  const tools = new Set(mode === "managed_data" ? [
    "mcp_pages_get_page_data", "mcp_pages_get_page_config", "mcp_pages_record_refresh_check",
    "mcp_pages_create_upload_ticket", "mcp_pages_update_page_data_upload", "mcp_pages_preflight_page",
  ] : []);
  for (const source of sources || []) {
...
function managedPrompt({ slug, instructions, schemaSha256, publish, recurring, sources = null }) {
  ...
  return [
    ...
    JSON.stringify(executionRequirements(sources, "managed_data")),
    "The scheduler must supply these capabilities. Network means sandbox HTTPS access for file upload, not merely working MCP calls; never bypass a sealed network policy.",
    ...
    `5. Validate source completeness, required fields, mappings, date grain and unmapped counts before deciding freshness. Missing, inaccessible, ambiguous or partial required sources select blocked. ${BLANK_COLUMN_CLAUSE}`,
    recurring
      ? "6. Compare internal source coverage with envelope.source_as_of and the periods already represented. If no source supplies missing coverage and no covered-period correction is requested, select source_not_updated. Otherwise select update. A file timestamp alone never justifies publication."
    ...
    "",
    "Completion audit: when your runtime requires confirm_audit, use critical_actions=[] if no mutations remain. Never declare an update-branch mutation for source_not_updated or blocked. Report blocked as blocked, not as an updated page.",
    "",
    "UPDATE BRANCH ONLY",
    ...
    "9. Over 20,000 UTF-8 bytes, use mcp_pages_create_upload_ticket kind='data' with the exact file byte count and lowercase SHA-256; PUT the file to the returned URL from your shell, then call mcp_pages_update_page_data_upload with upload_id. A smaller object may use mcp_pages_update_page_data inline. Never emit a large file or base64 through model output. If no permitted file transport works, select blocked and keep the file: do not restart/cancel upload loops, probe alternate hosts or weaken network policy. A transient upload failure allows one retry of the same exact bytes; stop on an explicit policy, authentication or DNS failure.",
    "10. Use this run's live_version_id as expected_version, source_as_of equal to the latest source date actually represented, the reconciled expect, and PUBLISH as specified. If the runtime provides confirm_audit, follow its protocol before each gated mutation and declare only actions this branch will execute. ",
...
function migrationPrompt({ slug, instructions, liveVersionId, publish, recurring, sources = null }) {
  return [
    ...
    `5. Only after a separately authorized migration publication, call mcp_pages_prepare_dashboard_update again with recurring=${recurring ? "true" : "false"}, update_type=data, publish=${publish ? "true" : "false"}, and the same FUTURE UPDATE REQUEST.`,
    "",
