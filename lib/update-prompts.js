...
function sourcesFromWorkflow(workflow) {
  ...
  for (const entry of raw.slice(0, MAX_SOURCES)) {
    ...
    // Same rule as normalizeSources, but a legacy read drops what it cannot use.
    if (!REQUIREMENT_IDENTIFIER.test(entry.mcp_server.trim())) continue;
...
function executionRequirements(sources, mode, { slug = null, recurring = false } = {}) {
  ...
  return {
    ...
    ...(managed ? {
      ...(recurring && rosterable(sources) ? { roster: "required_tools_only" } : {}),
      completion: { any_succeeded: [...COMMIT_TOOLS, "mcp_pages_record_refresh_check"].sort() },
...
function managedPrompt({ slug, instructions, schemaSha256, publish, recurring, sources = null }) {
  ...
  const requirements = executionRequirements(sources, "managed_data", { slug, recurring });
  return [
...
async function prepare({
  ...
  return {
    ...
    execution_requirements: executionRequirements(sources, mode, { slug, recurring: !!recurring }),
