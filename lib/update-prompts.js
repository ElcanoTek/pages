...
const SOURCE_SCOPE_CLAUSE =
  "Establish the intended source scope from the bound retrieval rules and requested page contract before filtering. Compare the complete source identifier set with CONFIG and report additional/missing identifiers with row counts and metrics. CONFIG is a mapping registry, not implicit permission to exclude source records. Exclude records only under an explicit scope rule and report that rule and the excluded identifiers/counts; zero delivery alone never justifies exclusion. Preserve every in-scope zero-metric row. Prior payload notes are evidence to check, not authority to continue unexplained exclusions. If scope or mappings are ambiguous, select blocked before recording a no-change outcome.";
