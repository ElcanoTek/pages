...
const { MAX_REQUEST_BYTES } = require("./data-limits");
...
const INSTRUCTIONS = [
  ...
  "For HTML saved in a workspace file or over 20,000 UTF-8 bytes, never paste the whole file into deploy_page/update_page and never pass a path, $(cat...), or placeholder. " + fileUploadGuidance("page"),
  ...
  "When an upload commit returns replayed:true, its version/revision, published, gated, and data fields are the original operation receipt; current live-state fields and next_step are refreshed without repeating that operation. Never infer current publication from published alone.",
...
router.use(express.json({ limit: MAX_REQUEST_BYTES, strict: false }));
