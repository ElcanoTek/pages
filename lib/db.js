...
// getLatestRenderable — the NEWEST version of a page, whatever its status. A deck
// edit session continues from the last save, not from what clients see: saves
// land as drafts, and an editor that reopened on the live version would show the
// author their own work undone.
async function getLatestRenderable(pageId) {
  const { rows } = await query(
    `SELECT v.id, v.html, v.render_mode, v.page_id, v.status,
            p.slug, p.disabled,
            t.override_css, t.default_mode
       FROM page_versions v
       JOIN pages p  ON p.id = v.page_id
       LEFT JOIN themes t ON t.id = p.theme_id
      WHERE v.page_id = $1 AND p.deleted_at IS NULL
