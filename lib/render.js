...
function renderVersion(renderable) {
  // A deck is served exactly as stored, whatever its row says: no theme and no
  // switcher. prepareDeploy refuses to store a themed deck, but a row can predate
  // that rule, and the correct render of a deck does not depend on how it was
  // filed.
  if (isBentoDeck(renderable.html)) return renderable.html;
