...
// A Bento deck — bento.page's one-file presentation, the .bento.html that
// fleet's bento-slides skill produces — is its own viewer AND editor. The runtime
// is a deflate-compressed base64 block that a bootstrap inflates and import()s
// from a blob URL; the slides are one JSON block. Pages recognises a deck by that
// block, which upstream's own tooling anchors on and fleet's tests pin as
// appearing exactly once.
//
// Recognition matters because a deck is an application, not a dashboard. It has
// to be served byte-for-byte: no Flag head (the deck carries a complete design
// of its own, and its CSP <meta> must stay the first thing in <head>), and no
// Page menu (the deck's own toolbar is exactly where the control would land).
const BENTO_DOC_RE =
  /<script\b[^>]*\btype\s*=\s*(?:"application\/bento\+json"|'application\/bento\+json'|application\/bento\+json(?=[\s>]))[^>]*>/i;
function isBentoDeck(sourceHtml) {
  return (
    typeof sourceHtml === "string" && sourceHtml.includes("application/bento+json") && BENTO_DOC_RE.test(sourceHtml)
  );
}
...
// half the live fleet.
// The one thing Pages adds to a deck. The content host's sandbox refuses every
// native file picker — Chromium: "Sandboxed documents aren't allowed to show a
// file picker" — and Bento chooses between "rewrite this file in place" and
// "download an updated copy" by whether window.showSaveFilePicker EXISTS. So on
// Chrome and Edge, Save reached for the picker, the browser refused, and nothing
// happened: no download, no message, a console error nobody sees. Making the API
// absent before the runtime boots puts Bento on its download path — which it
// labels itself: "This browser can't rewrite the open file" — and turns Add
// image and Replace from JSON into ordinary file inputs. Firefox and Safari have
// no such API and were on that path already. Idempotent, and stripped again at
// deploy time (stripDeckHostAdaptation), because a file saved from the hosted
// editor carries it and may well be uploaded back as the next version.
//
// Two more rules, both so that a file which LEAVES Pages is pure Bento: Bento
// serialises the live DOM when it saves, so anything still in <head> travels
// into the saved copy. (1) The script removes its own element once it has run,
// so a copy saved from the hosted editor does not carry it and, opened locally,
// behaves exactly as upstream (rewrite in place where the browser can). (2) It
// acts only when it is actually where it was written for — a sandboxed document
// (window.origin is "null") served over http(s) — so if a copy ever did carry
// it, a local file:// open or an unsandboxed host would leave the pickers alone.
const DECK_HOST_ADAPTATION =
  '<script data-pages-deck-host>(function(){var s=document.currentScript;' +
  'if(window.origin==="null"&&/^https?:$/.test(location.protocol)){' +
  'for(var k of ["showSaveFilePicker","showOpenFilePicker","showDirectoryPicker"]){' +
...
// Matches the tag as WRITTEN and as a browser re-serialises it: a file saved
// from the hosted editor comes back with `data-pages-deck-host=""`, and a strip
// that only knew the bare attribute left the tag in the stored bytes — so the
// next render served two.
const DECK_HOST_ADAPTATION_RE = /<script\b[^>]*\bdata-pages-deck-host\b[^>]*>[\s\S]*?<\/script\s*>/gi;

function adaptDeckToHost(html) {
  const clean = stripDeckHostAdaptation(html);
  // Right after <head>, ahead of the bootstrap at the end of <body>. A deck
  // without <head> does not exist (the vendored shell always has one); the
  // fallback still lands before the runtime boots, since the bootstrap inflates
  // the runtime asynchronously.
  // No whitespace of our own around the tag: the strip removes exactly the tag,
  // so the stored bytes of a re-uploaded saved copy are the original bytes.
  if (HEAD_OPEN_RE.test(clean)) return clean.replace(HEAD_OPEN_RE, (match) => `${match}${DECK_HOST_ADAPTATION}`);
  return appendToBody(clean, DECK_HOST_ADAPTATION);
}

// An edit session (lib/bento.js) also widens the deck's OWN guard meta from
// connect-src 'none' to this host's origin, and Bento serialises the live DOM on
// Save — so the file that comes back through the save channel carries the
// widened guard. Restore it here, with the tag strip, so the stored bytes keep
// fleet's guard exactly as fleet wrote it and a copy that later leaves Pages
// cannot reach anything, not even us.
const GUARD_META_RE = /<meta\b[^>]*http-equiv\s*=\s*["']Content-Security-Policy["'][^>]*>/i;
function stripDeckHostAdaptation(html) {
  if (typeof html !== "string") return html;
  let out = html.includes("data-pages-deck-host") ? html.replace(DECK_HOST_ADAPTATION_RE, "") : html;
  if (out.includes(`connect-src ${CONTENT_ORIGIN}`)) {
    out = out.replace(GUARD_META_RE, (tag) => tag.replace(`connect-src ${CONTENT_ORIGIN}`, "connect-src 'none'"));
  }
  return out;
}

function renderVersion(renderable) {
  // A deck is served as stored — whatever its row says, no theme and no switcher
  // — plus the one host adaptation above. prepareDeploy refuses to store a
  // themed deck, but a row can predate that rule, and the correct render of a
  // deck does not depend on how it was filed.
  if (isBentoDeck(renderable.html)) return adaptDeckToHost(renderable.html);
...
module.exports = {
  ...
  isBentoDeck,
  adaptDeckToHost,
  stripDeckHostAdaptation,
  DECK_HOST_ADAPTATION,
  NAV_BLOCK_ID,
  escapeStyleText,
};
