...
function rawHeaders() {
  const csp = [
    ...
    "default-src 'none'",
    // blob: — a document that carries its runtime compressed and boots it with
    // `import(URL.createObjectURL(new Blob([js])))`. A Bento deck is exactly
    // this: one deflate-b64 block and a bootstrap. Without blob: such a page dies
    // with "Failed to fetch dynamically imported module: blob:null/…", and
    // nothing static can warn about it, because the code it would warn about is
    // compressed. Threat model: 'unsafe-inline' is already here, so anyone who
    // can put a script tag into a page can already run any code they like —
    // blob: hands that attacker nothing new. What it hands a legitimate
    // document is module import() and Workers. connect-src 'none' still holds:
    // a blob is manufactured from bytes the page already has, never fetched.
    "script-src 'self' 'unsafe-inline' blob:",
    "style-src 'self' 'unsafe-inline'",
    // blob: — charts that rasterise to an object URL before drawing.
    "img-src 'self' data: blob:",
    // data:/blob: — audio and video embedded in the document, and only that. A
    // remote media URL is a beacon that reports who opened a page and when.
    "media-src data: blob:",
...
// Headers for a deck EDIT SESSION at /raw — the one response on the content host
// that may talk back to Pages. Identical to rawHeaders() except connect-src names
// this host's own origin, so the save channel can POST the edited deck and
// nothing else can be reached: not bento.page (the update check), not
// sync.bento.page (live collaboration), not anywhere. The sandbox is untouched,
// so the document is still an opaque origin with no cookies; the request is
// authorised by the edit token alone (server.js). A partner or client viewing
// the same deck never gets this header — they get rawHeaders(), connect-src
// 'none', and a Save that downloads.
function rawEditHeaders() {
  const headers = rawHeaders();
  headers["Content-Security-Policy"] = headers["Content-Security-Policy"].replace(
    "connect-src 'none'",
    `connect-src ${CONTENT_ORIGIN}`
  );
  return headers;
}
