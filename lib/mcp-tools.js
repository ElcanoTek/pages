...
function pageDataExport(result, ctx) {
  ...
  return {
    ...
    next_step:
      `Download schema_url and envelope_url (or data_url) once each into workspace files before ${minted.expiresAt} ` +
      "with a host-side URL download such as download_url, then read them from disk; do not print their contents. " +
      "Check sha256 of the schema file against schema_sha256 and of the data file against data_sha256. These URLs are " +
