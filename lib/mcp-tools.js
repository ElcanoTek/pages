...
const dataLimits = require("./data-limits");
const { normalizeSourceAsOf, boundedCoverageProfile } = require("./page-data");
...
const FreshnessSchema = z
  .object({
    ...
    last_check_at: NullableDateTime.describe("Timestamp of the explicit recorded check; its outcome may predate a successful refresh."),
    latest_outcome: z.string().nullable().describe("Outcome at checked_at: updated when the live refresh is newer, otherwise the recorded check outcome."),
    latest_detail: z.string().nullable(),
    latest_source_as_of: NullableDateTime,
    last_check_outcome: z.string().nullable().describe("Historical recorded check outcome at last_check_at, not necessarily the latest activity."),
...
// get_page_data answers three questions, and a refresh run asks the cheap one
// on every invocation. The shapes differ, but MCP requires an object-rooted
// output schema, so fields specific to one read mode are optional in the
// advertised schema and required or forbidden by the refinement below. A full
// read (no `detail`) validates exactly as it did before `detail` existed.
const PageDataPageBriefSchema = z
  .object({ slug: z.string(), title: z.string(), disabled: z.boolean(), require_approval: z.boolean() })
  .strict();

const PageDataExportsSchema = z
  .object({
    version_id: IdOut.describe("The immutable version these URLs serve: the live version when they were minted."),
    schema_url: z.string().url().describe("Canonical JSON of the data schema; its SHA-256 is schema_sha256."),
    data_url: z.string().url().describe("Canonical JSON of envelope.data; its SHA-256 is data_sha256."),
    envelope_url: z.string().url().describe("Canonical JSON of the complete envelope, including data."),
    expires_at: DateTime,
  })
  .strict();

const PAGE_DATA_MODE_FIELDS = Object.freeze({
  full: {
    required: ["schema", "version", "version_is_live", "page_is_live", "urls", "data_profile", "freshness"],
    forbidden: ["coverage_profile", "coverage_truncated", "exports", "next_step"],
  },
  summary: {
    required: ["coverage_profile", "freshness", "next_step"],
    forbidden: ["schema", "version", "version_is_live", "page_is_live", "urls", "data_profile", "data_omitted", "exports"],
  },
  export: {
    required: ["exports", "next_step"],
    forbidden: ["schema", "version", "version_is_live", "page_is_live", "urls", "data_profile", "data_omitted", "coverage_profile", "coverage_truncated", "freshness"],
  },
});

function refinePageDataMode(value, ctx) {
  const mode = value.detail || "full";
  const fields = PAGE_DATA_MODE_FIELDS[mode];
  for (const key of fields.required) {
    if (value[key] === undefined) ctx.addIssue({ code: "custom", path: [key], message: `a ${mode} read includes ${key}` });
  }
  for (const key of fields.forbidden) {
    if (value[key] !== undefined) ctx.addIssue({ code: "custom", path: [key], message: `a ${mode} read omits ${key}` });
  }
  if (mode !== "full" && hasOwn(value.envelope, "data")) {
    ctx.addIssue({ code: "custom", path: ["envelope", "data"], message: `a ${mode} read never carries envelope.data` });
  }
  if ((mode === "full") !== hasOwn(value.page, "id")) {
    ctx.addIssue({ code: "custom", path: ["page"], message: `a ${mode} read returns the ${mode === "full" ? "complete" : "brief"} page` });
  }
}
...
// Inline data and staged data use the same managed-data contract. Their
// transport limits are distinct from the materialized envelope limit.
const { MAX_INLINE_DATA_BYTES } = dataLimits;
...
// Committed uploads keep their original receipt. Only serving guidance is
// refreshed on replay, using the original entity ID so a reused slug/name can
// never turn an old receipt into a claim about its replacement.
async function replayPageUpload(client, receipt) {
  const page = (await client.query(
    `SELECT slug, disabled, deleted_at, published_version_id FROM pages WHERE id = $1`,
    [receipt.version.page_id]
  )).rows[0];
  const deleted = !page || !!page.deleted_at;
  const liveVersionId = deleted ? null : page.published_version_id;
  const pageIsLive = !deleted && !page.disabled && !!liveVersionId;
  const versionIsLive = pageIsLive && compareIds(receipt.version.id, liveVersionId);
  const prefix = `Upload was already committed as version ${receipt.version.id}. This retry did not republish or change it. `;
  let nextStep;
  if (deleted) {
    nextStep = "The original page was deleted and is not serving. Do not share its live URL; a reused slug may belong to another page.";
  } else if (page.disabled) {
    nextStep = "The page is disabled by an admin and is not serving. A human admin must re-enable it before its live URL can be shared.";
  } else if (versionIsLive) {
    nextStep = "This version is live now — share urls.live with the user or client.";
  } else {
    nextStep = pageIsLive
      ? `Version ${liveVersionId} is currently live; this upload's version is not. `
      : "No version is currently live. ";
    nextStep += Object.hasOwn(receipt, "envelope")
      ? "Call get_page_data to read the current state before deciding on any new data update or publication."
      : "Call get_page and list_versions to read the current state before deciding on any new publication.";
  }
  return {
    ...receipt,
    replayed: true,
    ...(Object.hasOwn(receipt, "live") ? { live: versionIsLive } : {}),
    version_is_live: versionIsLive,
    page_is_live: pageIsLive,
    live_version_id: liveVersionId || null,
    next_step: dataUpdateNextStep(prefix + nextStep, receipt.data_warnings || []),
  };
}

async function replayTemplateUpload(client, receipt) {
  const template = (await client.query(
    `SELECT t.deleted_at, v.revision
       FROM page_templates t LEFT JOIN page_template_versions v ON v.id = t.current_version_id
      WHERE t.id = $1`,
    [receipt.template.id]
  )).rows[0];
  const prefix = `Upload was already registered as ${receipt.template.name} revision ${receipt.revision.revision}. This retry did not register or change it. `;
  let nextStep;
  if (!template || template.deleted_at) {
    nextStep = "The original template was retired. A reused name may belong to another template; call list_templates before selecting a design.";
  } else if (Number(template.revision) !== receipt.revision.revision) {
    nextStep = `Its current revision is ${template.revision}. Call get_template to read the current design before creating or rerendering a page.`;
  } else {
    nextStep = templateNextStep(receipt);
  }
  return { ...receipt, replayed: true, next_step: prefix + nextStep };
}
...
async function registerTemplateUpload(args, ctx) {
  return pageUploads.deploy(
    ...
    { expectKind: "template", replayCommitted: replayTemplateUpload }
  );
}
...
async function deployPageUploadLocked(args, ctx) {
  ...
  }, { replayCommitted: replayPageUpload });
}
...
// The two compact reads. Neither carries the schema or a row.
const SUMMARY_NEXT_STEP =
  "Enough to pick a branch. For the schema or rows, call get_page_data once with detail='export' and download " +
  "the URLs into files (e.g. download_url), or detail='full' once if you cannot. live_version_id is expected_version.";

const SUMMARY_TRUNCATED_STEP =
  "coverage_profile is partial (coverage_truncated): take complete row counts and date ranges from the exported data file.";

// A recorded check's detail may be 500 characters; two of them would be most of
// a summary's budget. The complete lines stay on a full read.
const SUMMARY_DETAIL_CHARS = 160;
function clipDetail(detail) {
  return typeof detail === "string" && detail.length > SUMMARY_DETAIL_CHARS
    ? `${detail.slice(0, SUMMARY_DETAIL_CHARS - 1)}…`
    : detail;
}

function briefFreshness(freshness) {
  if (!freshness) return freshness;
  return { ...freshness, latest_detail: clipDetail(freshness.latest_detail), last_check_detail: clipDetail(freshness.last_check_detail) };
}

// MCP and setTitle cap a title at 200 characters, but POST /api/v1/pages does
// not, so a legacy page can carry a far longer one. The compact reads promise a
// small answer, so they clip it (with an ellipsis); a full read keeps it whole.
const BRIEF_TITLE_CHARS = 200;
function pageBrief(page) {
  const title = typeof page.title === "string" && page.title.length > BRIEF_TITLE_CHARS
    ? `${page.title.slice(0, BRIEF_TITLE_CHARS - 1)}…`
    : page.title;
  return { slug: page.slug, title, disabled: page.disabled, require_approval: page.require_approval };
}

function pageDataSummary(result) {
  const { data: _data, ...stamps } = result.envelope;
  const coverage = boundedCoverageProfile(result.data_profile);
  return {
    detail: "summary",
    page: pageBrief(result.page),
    live_version_id: result.page.published_version_id || null,
    data_sha256: result.data_sha256,
    schema_sha256: result.schema_sha256,
    template_sha256: result.template_sha256,
    envelope: stamps,
    freshness: briefFreshness(result.freshness),
    coverage_profile: coverage.profile,
    ...(coverage.truncated ? { coverage_truncated: true } : {}),
    next_step: coverage.truncated ? `${SUMMARY_NEXT_STEP} ${SUMMARY_TRUNCATED_STEP}` : SUMMARY_NEXT_STEP,
  };
}

function pageDataExport(result, ctx) {
  const { data: _data, ...stamps } = result.envelope;
  const minted = dataExport.mint({ pageId: result.page.id, versionId: result.version.id, tokenId: ctx && ctx.tokenId });
  return {
    detail: "export",
    page: pageBrief(result.page),
    live_version_id: result.page.published_version_id || null,
    ...
    template_sha256: result.template_sha256,
    envelope: stamps,
    exports: { version_id: String(result.version.id), ...dataExport.exportUrls(minted.token), expires_at: minted.expiresAt },
    next_step:
      `Download schema_url and data_url once each into workspace files before ${minted.expiresAt} ` +
      "with a host-side URL download such as download_url, then read them from disk; do not print their contents. " +
      "Check sha256 of each downloaded file's exact bytes against schema_sha256 and data_sha256 (do not re-serialize to hash); " +
      "envelope_url is optional, since the stamps are above. These URLs are " +
      "credentials for this one version: never share or repeat them. Mint new ones only if these expired before download.",
  };
}

async function getPageData(args, ctx) {
  const result = await versions.getPageData(args.slug);
  if (args.detail === "summary") return pageDataSummary(result);
  if (args.detail === "export") return pageDataExport(result, ctx);
  const { data, ...coverage } = result.envelope;
  const omitData = args.include_data === false;
  return {
    page: result.page,
    version: result.version,
...
function assertInlineData(data, sourceAsOf) {
  if (!data || typeof data !== "object" || Array.isArray(data)) {
    throw badRequest("data must be a JSON object", "data_invalid");
  }
  const measured = dataLimits.measureData(data);
  const bytes = measured.payload_bytes;
  dataLimits.assertDomainSize(bytes, "payload");
  const normalizedSource = sourceAsOf === undefined ? undefined : normalizeSourceAsOf(sourceAsOf);
  dataLimits.assertDomainSize(measured.escaped_payload_bytes + dataLimits.envelopeOverhead(normalizedSource), "envelope");
  if (bytes > MAX_INLINE_DATA_BYTES) {
    const canStage = bytes <= dataLimits.MAX_UPLOAD_BYTES;
    throw badRequest(
      `data is ${bytes} bytes; inline managed-data transport is capped at ${MAX_INLINE_DATA_BYTES}. ` +
        (canStage ? "Stage the complete compact JSON file: create_upload_ticket with kind 'data', PUT it from your shell, then update_page_data_upload. "
          : "The compact file also exceeds the staged upload limit. Keep the complete source and ask the operator to review capacity. ") +
        "Do NOT split, sample or summarize the payload to fit. " + dataLimits.dataLimitGuidance(),
      "data_too_large_for_inline",
      { bytes, max_bytes: MAX_INLINE_DATA_BYTES, changing_transport_can_help: canStage, limits: dataLimits.DATA_LIMITS }
    );
  }
  return bytes;
}
...
// no inline transport ceiling to hit. Data/envelope limits still apply. From
// the parse onward it is the same write, through versions.updatePageDataWithClient, inside the upload's own
...
async function updatePageDataUpload(args, ctx) {
  return pageUploads.deploy(
    ...
    { expectKind: "data", replayCommitted: replayPageUpload }
  );
}
...
async function updatePageData(args, ctx) {
  assertInlineData(args.data, args.source_as_of);
...
const UploadReplaySchema = z.literal(true).optional().describe(
  "Present on an exact committed retry. Commit receipt fields describe the original operation; live-state fields and next_step are refreshed for this reply. The mutation was not repeated."
);
...
const DataLimitsSchema = z.object(Object.fromEntries(
  Object.keys(dataLimits.DATA_LIMITS).map((key) => [key, z.number().int().positive()])
)).strict();
const DataSizeSchema = z.object({
  payload_bytes: z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER),
  escaped_payload_bytes: z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER),
}).strict().describe("Advisory UTF-8 sizes of compact JSON and HTML-escaped compact JSON; only for kind data. Actual content is always validated.");
...
const UploadStateSchema = z
  .object({
    ...
    data_limits: DataLimitsSchema.optional(),
    complete: z.boolean(),
    expires_at: DateTime,
    next_step: z.string(),
  })
...
const DeployInputShape = {
  ...
  html: z
    ...
    .min(1)
    .describe(
      "Inline page HTML or fragment. For a workspace file or content over 20,000 UTF-8 bytes, do not inline it. " + fileUploadGuidance("page")
    ),
  render_mode: z.enum(["themed", "raw"]).optional().describe("Defaults to themed. A Bento deck (.bento.html) defaults to raw and is refused as themed: it is its own application."),
  ...
  .object({
    ...
    execution_requirements: z
      .object({
        ...
        // Managed-data runs only. Schedulers that do not know a key ignore it.
        roster: z
          .literal("required_tools_only")
          .optional()
          .describe("Recurring managed data with fully bound sources only: offer the run only required_tools; every tool a branch may call is listed."),
        completion: z
          .object({ any_succeeded: z.array(z.string()).min(1) })
          .strict()
          .optional()
          .describe("Recurring managed data only: the run is complete when any one of these tools succeeded: a committed version or a recorded check."),
        serialization_key: z
          .string()
          .optional()
          .describe("pages:<slug>. Runs sharing it should not overlap; give every schedule for this page the same key."),
        network: z.boolean(),
...
const TOOLS = {
  ...
  get_page_data: {
    ...
    description:
      "Read the currently published managed-data contract. Use its live_version_id as update_page_data.expected_version. Fails with page_not_data_managed unless the published HTML has exactly one pages-data-schema block and one pages-data block.\n\n" +
      "detail picks how much comes back. 'summary' (~1–2 KB): brief page flags, live_version_id, the three hashes, envelope stamps, freshness and a coverage profile (per-array row counts, date field min/max/distinct, date-stamped scalars). Read it first; it is enough to decide whether a refresh is due. 'export': short-lived URLs for the schema, data and envelope of the live version, for a client that downloads URLs into workspace files host-side; the bytes then never enter your context, and sha256 of the schema/data files equals schema_sha256/data_sha256. 'full' (default): today's complete answer — schema, envelope with data, full data_profile, URLs and live state. Read the schema or rows once per run, by export where you can; do not re-read after a compaction.\n\n" +
    ...
    inputSchema: z
      .object({
        slug: Slug,
        detail: z
          .enum(["summary", "full", "export"])
          .optional()
          .describe("Default 'full'. 'summary' for the compact contract read; 'export' for download URLs instead of inline schema/data."),
        include_data: z
          .boolean()
          .optional()
          .describe("Full reads only. Default true. False returns the schema, hashes, profile, live state and coverage without envelope.data."),
      })
      .strict(),
    outputSchema: z
      .object({
        detail: z.enum(["summary", "export"]).optional().describe("Absent on a full read."),
        page: z.union([PageSchema, PageDataPageBriefSchema]),
        version: VersionSchema.optional(),
        schema: JsonObjectSchema.optional(),
        envelope: DataEnvelopeSchema.partial({ data: true }),
        data_sha256: Sha256,
        schema_sha256: Sha256,
        template_sha256: Sha256,
        version_is_live: z.boolean().optional(),
        page_is_live: z.boolean().optional(),
        live_version_id: IdOut.nullable(),
        urls: UrlsSchema.optional(),
        data_omitted: z.literal(true).optional(),
        data_profile: DataProfileSchema.optional(),
        coverage_profile: DataProfileSchema.optional().describe(
          "Summary reads: data_profile reduced to row counts, date fields and date-stamped scalars."
        ),
        coverage_truncated: z
          .literal(true)
          .optional()
          .describe("Summary reads: coverage_profile is incomplete (size bound, a clipped extent, or the profile's array/field/scalar/depth limits); the exported data file is complete."),
        freshness: FreshnessSchema.optional(),
        exports: PageDataExportsSchema.optional(),
        next_step: z.string().optional(),
      })
      .strict()
      .superRefine(refinePageDataMode),
    annotations: ReadAnnotations,
    handler: getPageData,
  },
  ...
  create_upload_ticket: {
    ...
    inputSchema: z
      .object({
        ...
        data_size: DataSizeSchema.optional(),
  ...
  start_page_upload: {
    ...
    description:
      dataLimits.dataLimitGuidance() + "Stage exact file bytes for ordered MCP appends. Prefer this path when your client advertises workspace_file references on append_page_upload.chunk_base64: the client reads and encodes the file, with no model-generated base64 and no shell credential. Supply the exact whole-file byte count and lowercase SHA-256, append the original bytes in sequence in as few appends as the returned max_chunk_bytes allows, then call the matching upload consumer. Staging does not publish. An append failure does not advance the sequence; retry the same exact bytes. Do not cancel and restart a usable upload. Clients without file-reference support need another permitted programmatic transport; create_upload_ticket supports direct file HTTP. Never print or transcribe a large base64 payload through model output.",
    inputSchema: z
      .object({
        slug: Slug.optional().describe("The page this upload targets. Supply exactly one of slug or template."),
        template: TemplateName.optional().describe(
  ...
  append_page_upload: {
    ...
    description:
      "Append one ordered base64 chunk to a staged upload. If your client advertises a workspace_file reference for chunk_base64, use it to transmit exact file bytes without printing or generating base64. Decode size must not exceed max_chunk_bytes from start_page_upload; each append is a full round trip, so use as few appends as max_chunk_bytes allows. Send the exact next_sequence; replaying the same sequence and the same bytes is a safe no-op.\n\nA FAILED append does not advance the sequence and does not lose anything already accepted, so every error here is resumable: re-send from the expected_sequence the error reports. Do not cancel to recover — that deletes every byte you have already emitted and makes you send the whole document again, which is how a single page can consume a whole turn.\n\nPreserve original byte order; do not minify or alter content between chunks.",
    inputSchema: z
      .object({
        upload_id: UploadId,
  ...
  create_template_from_page: {
    ...
    outputSchema: z
      .object({
        created: z.boolean(),
        deduped: z.boolean(),
        metadata_updated: z.boolean().describe("The existing template title or description changed; the mutation was audited."),
        template: TemplateSchema,
        revision: TemplateRevisionSchema,
        config_schema: JsonObjectSchema,
        data_schema: JsonObjectSchema,
  ...
  rerender_page_from_template: {
    ...
    description:
      "Move ONE page onto a different revision of its template, or pull a DRIFTED page back onto it, keeping its own config and data by default. For a schema migration, supply complete target-valid config and/or data in this same call, with expected_version. Replacement data requires explicit source_as_of and records a fresh ingestion; omitted data preserves the entire live envelope. A page a raw deploy_page or patch_page detached (list_template_pages: drifted:true) has no binding left to infer the design from, so name the template explicitly and it is re-attached \u2014 which works as long as its published HTML still carries #pages-config and #pages-data, the usual case after patch_page. Any omitted config or data comes from the page's published HTML, and all values are validated against the target revision's schemas, so a revision that tightened its contract fails loudly instead of producing a page its own schema rejects. publish defaults to FALSE: the new design lands as an inspectable version so a human can preview it before a client sees it, then publish_page makes it live. One page per call — there is no bulk re-render.",
    inputSchema: z
      .object({
        slug: Slug,
        ...
        config: JsonObjectSchema.optional().describe("Complete replacement config validated against the TARGET revision; never merged with reference config. Requires expected_version."),
        data: JsonObjectSchema.optional().describe("Complete replacement data validated against the TARGET revision. Requires source_as_of and expected_version."),
        source_as_of: DateTime.optional().describe("Required only with replacement data; source coverage cannot regress."),
        publish: z
          .boolean()
          .optional()
  ...
  deploy_page_upload: {
    ...
    description:
      "Verify and atomically deploy a complete staged upload, creating the page if needed and publishing by default on open pages. The upload slug is fixed by start_page_upload. Exact retries preserve the original commit receipt, add replayed:true, and refresh current live-state fields and next_step without repeating the mutation. Approval, optimistic concurrency, immutable versions, and audit behavior match deploy_page.",
    inputSchema: z.object(UploadDeployInputShape).strict(),
    outputSchema: DeployOutputSchema.extend({ upload_id: UploadId, created: z.boolean(), replayed: UploadReplaySchema }).strict(),
    annotations: IdempotentWriteAnnotations,
  ...
  update_page_data_upload: {
    ...
    description:
      dataLimits.dataLimitGuidance() + "Publish a managed-data payload you staged with create_upload_ticket (kind 'data') instead of sending it inline. The JSON never passes through your context. Payload, escaped-envelope and staged-file limits still apply. Everything else is update_page_data exactly: same schema validation, same monotonic source_as_of, same mandatory expected_version, same dedupe, same data_profile / data_warnings, same expect reconciliation — one write path, not two.\n\nThe staged bytes must parse as a JSON object. An upload staged as 'page' is refused here, and one staged as 'data' is refused by deploy_page_upload. Exact committed retries add replayed:true, preserve the original data receipt, and refresh current live-state fields and next_step without updating data again.",
    inputSchema: z
      .object({
        upload_id: UploadId,
      ...
      .object({
        ...
        data: JsonObjectSchema.describe(
          "Complete data object that satisfies the page's returned schema; never include credentials. "
            + "SEND IT WHOLE. Do not split it across calls, sample it, or decline to send because it looks "
            + "large. " + fileUploadGuidance("data") + " A partial payload passes schema "
            + "validation while publishing wrong numbers. Over the cap you get an explicit "
            + "data_validation_failed for payload/envelope limits or data_too_large_for_inline for the inline transport limit; nothing is truncated."
        ),
