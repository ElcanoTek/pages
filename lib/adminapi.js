...
router.get("/pages/*slug", h(async (req, res) => {
  ...
  // Whether the page's newest version is a Bento deck decides which editor the
  // admin offers: a deck opens in Bento with a save channel back to Pages;
  // anything else gets the source textarea. One regex over one html column,
  // computed here rather than guessed at from a filename the admin never sees.
  const newest = all.length ? await versions.getVersion(slug, all[0].id) : null;
  const deck = Boolean(newest && render.isBentoDeck(newest.html));
  res.json({
    page,
    deck,
    published,
...
// edit-token — open a Bento deck page in its own editor with a save channel back
// to Pages. Staff only (admin cookie + CSRF); the token names the actor so every
// save's audit row names a person; bound to the page, not a version, because
// every save makes a new one; eight hours, which is an editing session and not a
// standing credential. Refused unless the page's newest version IS a deck: the
// channel exists for one thing, and a view token is what every other page gets.
router.post("/pages/*slug/edit-token", h(async (req, res) => {
  const slug = slugOf(req);
  const { page } = await versions.getPage(slug);
  const latest = await db.getLatestRenderable(page.id);
  if (!latest || !render.isBentoDeck(latest.html)) {
    throw badRequest("only a Bento deck opens in its own editor; every other page has Edit source", "not_a_bento_deck");
  }
  const token = rawtoken.mint(
    { pageId: page.id, versionId: latest.id, purpose: "edit", renderMode: "raw", actor: req.user.email },
    8 * 3600
  );
