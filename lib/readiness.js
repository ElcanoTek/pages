...
async function probe() {
  ...
  try {
    ...
    await client.query({ text: "SELECT p.slug, p.published_version_id, v.html, t.default_mode FROM pages p LEFT JOIN page_versions v ON v.id=p.published_version_id LEFT JOIN themes t ON t.id=p.theme_id WHERE false", query_timeout: 1000 });
    const { rows } = await client.query({ text: "SELECT filename FROM schema_migrations WHERE filename=ANY($1::text[])", values: [migrations], query_timeout: 1000 });
    return rows.length === migrations.length;
  } catch { if (client) { client.release(true); client = null; } return false; }
