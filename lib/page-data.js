...
const { MAX_DATA_BYTES, escapedJson, assertDomainSize } = require("./data-limits");
...
// OMITTED_PROFILES holds each profile a cap above cut short. Kept beside the
// object rather than on it, so the full read's data_profile stays byte-identical
// and schema-valid (a key of any kind on a z.record would fail validation),
// while the summary read can still say its coverage is partial.
const OMITTED_PROFILES = new WeakSet();

function profileWasCapped(profile) {
  return OMITTED_PROFILES.has(profile);
}

function profileRows(rows, onCapped = () => {}) {
  const fields = new Map();
  const field = (name) => {
    let f = fields.get(name);
    if (!f) {
      if (fields.size >= PROFILE_MAX_FIELDS) {
        onCapped();
        return null;
      }
...
function profileData(data) {
  ...
  const walk = (value, path, depth) => {
    ...
    if (Array.isArray(value)) {
      ...
      if (objectRows > 0 && arraysProfiled < PROFILE_MAX_ARRAYS) {
        arraysProfiled += 1;
        arrays[path] = { count: value.length, fields: profileRows(value, () => { omitted = true; }) };
    ...
    if (path && (typeof value !== "object" || value === null)) {
      if (scalarsRecorded < PROFILE_MAX_SCALARS) {
        scalarsRecorded += 1;
        scalars[path] = value;
      } else {
        omitted = true;
  ...
  walk(data, "", 0);
  const profile = { arrays, scalars };
  if (omitted) OMITTED_PROFILES.add(profile);
  return profile;
}

// coverageProfile — the part of a profile that decides whether a refresh is due:
// how many rows each array holds and which dates they span, plus date-stamped
// scalars (dataThrough, lastRefreshed). Numeric sums, key values and free-text
// scalars are dropped. A refresh run reads this on every invocation, and the
// full profile of a wide payload is tens of kilobytes that every later model
// step re-sends. Same shape as profileData, so a caller reads it the same way.
//
// It is also BOUNDED, because the summary read promises a small answer whatever
// the payload holds. The full profile keeps date extents verbatim, so a string
// that merely starts with a date can make min and max each hundreds of
// kilobytes, and eight arrays of 24 date fields is a few hundred entries. So a
// retained value is clipped to COVERAGE_VALUE_LEN (an ISO datetime with offset
// fits), names longer than COVERAGE_NAME_LEN are skipped, each array keeps at
// most COVERAGE_MAX_DATE_FIELDS date fields, and the serialized result never
// exceeds COVERAGE_MAX_BYTES: entries that would cross it are left out (an array
// first keeps its count without fields) and `truncated` says so.
const COVERAGE_VALUE_LEN = 32;
const COVERAGE_NAME_LEN = 80;
const COVERAGE_MAX_DATE_FIELDS = 6;
const COVERAGE_MAX_SCALARS = 8;
const COVERAGE_MAX_BYTES = 3072;

function clipCoverageValue(value) {
  return typeof value === "string" && value.length > COVERAGE_VALUE_LEN
    ? `${value.slice(0, COVERAGE_VALUE_LEN - 1)}…`
    : value;
}

// → { profile: {arrays, scalars}, truncated: boolean }. truncated is true when
// ANY coverage is missing: cut by profileData's own caps (a ninth array, the
// 25th scalar or field, a level past PROFILE_MAX_DEPTH), clipped here, or left
// out to stay within the byte bound.
function boundedCoverageProfile(profile, maxBytes = COVERAGE_MAX_BYTES) {
  const out = { arrays: {}, scalars: {} };
  let truncated = profileWasCapped(profile);
  const clip = (value) => {
    const clipped = clipCoverageValue(value);
    if (clipped !== value) truncated = true;
    return clipped;
  };
  const fits = () => Buffer.byteLength(JSON.stringify(out)) <= maxBytes;
  for (const [path, entry] of Object.entries(profile.arrays)) {
    if (path.length > COVERAGE_NAME_LEN) { truncated = true; continue; }
    const fields = {};
    let kept = 0;
    for (const [name, field] of Object.entries(entry.fields)) {
      if (field.kind !== "date") continue;
      if (name.length > COVERAGE_NAME_LEN || kept >= COVERAGE_MAX_DATE_FIELDS) { truncated = true; continue; }
      fields[name] = { ...field, min: clip(field.min), max: clip(field.max) };
      kept += 1;
    }
    out.arrays[path] = { count: entry.count, fields };
    if (fits()) continue;
    truncated = true;
    // Keep what fits of this array: its count, then its fields one by one.
    out.arrays[path] = { count: entry.count, fields: {} };
    if (!fits()) { delete out.arrays[path]; continue; }
    for (const [name, field] of Object.entries(fields)) {
      out.arrays[path].fields[name] = field;
      if (!fits()) { delete out.arrays[path].fields[name]; break; }
    }
  }
  let scalarsKept = 0;
  for (const [path, value] of Object.entries(profile.scalars)) {
    if (!isDateLike(value)) continue;
    if (path.length > COVERAGE_NAME_LEN || value.length > PROFILE_MESSAGE_VALUE_LEN || scalarsKept >= COVERAGE_MAX_SCALARS) {
      truncated = true;
      continue;
    }
    out.scalars[path] = value;
    if (fits()) { scalarsKept += 1; continue; }
    delete out.scalars[path];
    truncated = true;
  }
  return { profile: out, truncated };
}

function coverageProfile(profile) {
  return boundedCoverageProfile(profile).profile;
}
...
function validatePayload(validate, value, { errorCode, label = "data", maxBytes = MAX_DATA_BYTES }) {
  ...
  if (label === "data") assertDomainSize(utf8Bytes(serialized), "payload", errorCode);
...
function materializeBlocks(managed, { data, config, envelope: preservedEnvelope } = {}, { sourceAsOf, now = Date.now() } = {}) {
  const replacements = [];
  ...
  if (preservedEnvelope !== undefined) {
    if (data !== undefined) throw badRequest("provide data or a preserved envelope, not both", "data_validation_failed");
    assertEnvelope(preservedEnvelope);
    data = preservedEnvelope.data;
  }
  if (data !== undefined) {
    assertRepresentableNumbers(data, "data_validation_failed", "data");
    validateData(managed.validate, data, "data_validation_failed");
    envelope = preservedEnvelope !== undefined ? preservedEnvelope : {
      contract_version: 1,
    ...
    assertDomainSize(utf8Bytes(encoded), "envelope");
