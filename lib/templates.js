...
async function registerWithClient(
  ...
  let template = (
    await client.query(
      `SELECT id, name, title, description, current_version_id, created_at, updated_at
  ...
  const metadataUpdated = !created && ((title !== null && title !== template.title) ||
    (description !== null && description !== template.description));
  const updated = deduped && !metadataUpdated ? template : (
    await client.query(
  ...
  if (!deduped || metadataUpdated) {
    await audit.write(client, {
      ...actorCtx,
      action: deduped ? "update_template_metadata" : created ? "create_template" : "register_template",
      metadata: {
        template: name,
        ...(metadataUpdated ? {
          before: { title: template.title, description: template.description },
...
async function createPage(
  ...
  return db.withTransaction(async (client) => {
    ...
    const assertExistingPage = async (existing) => {
      // The create-or-deploy helper invokes this only after locking the final
      // existing page, including when another transaction won the INSERT. An
      // earlier SELECT cannot protect an absent slug from a concurrent create.
      ...
      // and config; render mode and template revision must also match so a
      // different build cannot qualify as a retry on identical payloads.
      //
      ...
      const sameBuild = await client.query(
        `SELECT 1 FROM page_versions
          WHERE page_id = $1 AND data_sha256 = $2 AND data_template_sha256 = $3
            ...
            AND render_mode = $6 AND template_version_id = $7
            AND id = COALESCE(
              $5::bigint,
...
// data unless complete target replacements are supplied. Published HTML supplies
// the omitted blocks: nothing is re-derived
...
async function rerenderPage(
  { slug, template = null, revision = null, expectedVersion = null, publish = false, note = null,
    config, data, sourceAsOf = null },
  actorCtx
) {
  const normalizedSlug = versions.normalizeSlug(slug);
  if ((config !== undefined || data !== undefined) && expectedVersion == null) {
    throw badRequest("expected_version is required when migrating template config or data", "expected_version_required");
  }
  if (data !== undefined && sourceAsOf == null) {
    throw badRequest("source_as_of is required with replacement migration data", "source_as_of_required");
  }
  if (data === undefined && sourceAsOf != null) {
    throw badRequest("source_as_of requires replacement migration data", "data_required");
  }
  ...
  return db.withTransaction(async (client) => {
    ...
    const materialized = pageData.materializeBlocks(target, {
      config: config === undefined ? live.config : config,
      ...(data === undefined ? { envelope: live.envelope } : { data }),
    }, { sourceAsOf });

    if (data !== undefined) {
      // Replacement data follows the same coverage floor as update_page_data,
      // including unpublished canaries. The page lock serializes both paths.
      const floor = (await client.query(
        `SELECT max(source_as_of) AS source_as_of FROM page_versions
          WHERE page_id = $1 AND source_as_of IS NOT NULL AND status <> 'rejected'`,
        [page.id]
      )).rows[0].source_as_of;
      const sourceFloor = Math.max(Date.parse(live.envelope.source_as_of), floor ? new Date(floor).getTime() : 0);
      if (Date.parse(materialized.envelope.source_as_of) < sourceFloor) {
        throw conflict("source_as_of cannot move behind existing source coverage", "source_regression");
      }
    }
    ...
    const binding = (await client.query(
      `SELECT tv.id, tv.revision, t.name, pv.html FROM page_versions pv
         JOIN page_template_versions tv ON tv.id = pv.template_version_id
         JOIN page_templates t ON t.id = tv.template_id WHERE pv.id = $1`,
      [result.version.id]
    )).rows[0];
    // Exact retries reuse immutable bytes, including the refresh timestamp.
    const stored = pageData.parseManaged(binding.html, pageData.TEMPLATE_SPEC);
    return {
      ...result,
      ...
      template_revision: Number(binding.revision),
      template_version_id: String(binding.id),
      config: stored.config,
      config_sha256: stored.config_sha256,
      envelope: stored.envelope,
      data_sha256: stored.data_sha256,
      template_sha256: stored.template_sha256,
      html: binding.html,
    };
  });
}
