...
//
// Deliberately STATIC (parse5 + syntax parsers; authored code is never run). No browser is
...
const { sandboxTokens, rawHeaders, CONTENT_ORIGIN } = require("./csp");
...
// Read the exact response policy instead of maintaining a second permission
// list. base-uri 'none' means an authored <base> cannot change this origin.
const RESOURCE_POLICY = new Map(rawHeaders()["Content-Security-Policy"].split(";").map(part => {
  const [directive, ...sources] = part.trim().split(/\s+/);
  return [directive, sources];
}));

function resourceUrl(value, directive) {
  if (typeof value !== "string" || !value.trim()) return null;
  let url;
  try { url = new URL(value.trim(), CONTENT_ORIGIN); } catch { return { allowed: false, host: "an invalid URL" }; }
  const sources = RESOURCE_POLICY.get(directive) || RESOURCE_POLICY.get("default-src") || [];
  const allowed = sources.some(source =>
    source === "'self'" ? url.origin === new URL(CONTENT_ORIGIN).origin && /^https?:$/.test(url.protocol)
      : /^[a-z][a-z0-9+.-]*:$/i.test(source) && url.protocol === source.toLowerCase());
  return { allowed, host: url.host || url.protocol };
}

function checkSubresources(doc, cssText) {
  const errors = [];
  function check(value, directive, element, description) {
    const resource = resourceUrl(value, directive);
    if (!resource || resource.allowed) return;
    errors.push(finding("remote_subresource_blocked", `${description} references ${resource.host}, which the content host's "${directive}" blocks. It will not load.`, {
      element, host: resource.host, directive,
      fix: `Use an asset source permitted by ${directive}: ${(RESOURCE_POLICY.get(directive) || RESOURCE_POLICY.get("default-src")).join(" ")}. Do not widen the content policy to hide this finding.`,
    }));
  }
  walk(doc, node => {
    for (const rule of SUBRESOURCE_RULES) {
      if (node.nodeName === rule.tag) check(attr(node, rule.attr), rule.directive, rule.tag, `<${rule.tag}> ${rule.kind}`);
    }
    if (node.nodeName === "video") check(attr(node, "poster"), "img-src", "video", "Video poster");
    if (node.nodeName === "source" && ["audio", "video"].includes(node.parentNode?.nodeName)) {
      check(attr(node, "src"), "media-src", "source", "Media source");
    }
    if (node.nodeName === "link") {
      const rel = (attr(node, "rel") || "").toLowerCase().split(/\s+/);
      let directive;
      if (rel.includes("stylesheet")) directive = "style-src";
      else if (rel.includes("modulepreload")) directive = "script-src";
      else if (rel.includes("icon")) directive = "img-src";
      else if (rel.includes("preload")) directive = {
        script: "script-src", style: "style-src", image: "img-src", font: "font-src",
        audio: "media-src", video: "media-src", fetch: "connect-src",
      }[(attr(node, "as") || "").toLowerCase()];
      if (directive) check(attr(node, "href"), directive, "link", `<link rel="${rel.join(" ")}">`);
    }
  });

  const cssUrls = (css, directive) => {
    for (const match of css.matchAll(/url\(\s*['"]?([^'")]+)['"]?\s*\)/gi)) {
      check(match[1], directive, "css", "CSS url()");
    }
  };
  // Font sources and imports have different scheme permissions from images.
  // Remove the recognized blocks before scanning the remaining image URLs.
  const images = cssText.replace(/@font-face\s*\{[^}]*\}/gi, block => {
    cssUrls(block, "font-src");
    return "";
  }).replace(/@import\s+(?:url\(\s*)?['"]?([^'"\s);]+)['"]?\s*\)?[^;]*;/gi, (_, value) => {
    check(value, "style-src", "css", "CSS @import");
    return "";
  });
  cssUrls(images, "img-src");
  return errors;
}
...
// Both raw and themed pages receive navigation when a portal authorises the
// request. Source-only preflight cannot know that future request's credentials,
// so a reader is not itself a defect. Decks deliberately receive neither the
// payload nor the menu; use the renderer's predicates so guidance follows the
// same exception and does not mistake a prose/comment mention for a reader.
function checkNavBlock(html) {
  if (!render.isBentoDeck(html) || !render.readsNavBlock(html)) return [];
  return [
    finding(
      "nav_block_ignored",
      "This Bento deck reads #pages-nav, but Pages deliberately injects neither the navigation payload nor the Page menu into a deck, even when a portal authorises the view.",
      {
        fix:
          "Keep render_mode raw. Remove the custom switcher reader, or guard a missing block and render nothing; " +
          "the deck keeps its own toolbar. Return to the portal index to open another page.",
      }
    ),
  ];
}
...
// ── R8: a Bento deck ─────────────────────────────────────────────────────────
// Every rule above reads plaintext. A Bento deck (bento.page; what fleet's
// bento-slides skill produces) keeps its 650KB runtime deflate-compressed in a
// `type="bento/deflate-b64"` block and boots it through a blob: module import,
// so on a deck the rules above see one tiny bootstrap and fleet's guard script
// and report a clean page. That is exactly what preflight did while the deck
// could not start at all (blob: scripts were not in the CSP). So: say what a
// deck is and what to expect of it, and do not pretend the plaintext rules
// judged the application.
function checkBentoDeck(html, renderMode) {
  if (!render.isBentoDeck(html)) return { deck: false, errors: [], warnings: [] };
  const errors = [];
  if (renderMode !== "raw") {
    errors.push(
      finding(
        "bento_deck_themed",
        'This is a Bento deck and render_mode is "themed". A deck is its own application with a complete design; themed would splice Flag\'s head tags ahead of the deck\'s own CSP meta and draw a Page menu over its toolbar. Pages refuses to store a deck this way.',
        { fix: 'Omit render_mode — a deck defaults to raw — or pass "raw".' }
      )
    );
  }
  const warnings = [
    finding(
      "bento_deck",
      "This is a Bento deck. Its runtime is deflate-compressed, so the static checks here could not see the application and judged only its bootstrap. On the content host the deck opens in Bento's full editor. Nothing edited there is saved to Pages: Save downloads a new .bento.html (the sandbox refuses native file pickers, so Pages removes that API before the deck boots and Bento takes its download path — it says so on the button), Export PDF prints, and unsaved edits are gone on reload (the sandbox has no storage). Speaker view needs pop-ups and copy-to-clipboard needs a permission, neither of which the sandbox grants. Embedded video and audio must be data: or blob: URLs.",
      { fix: "Nothing to fix. To change the deck, edit it with fleet's bento_doc.py and deploy the new file as a new version." }
    ),
  ];
  // Bento mints a live-collaboration block — room URL and private key — into
  // any file saved from its own UI, and a file saved from the hosted editor is
  // exactly what comes back as the next version. Inert here: the deck's own CSP
  // and this host's both forbid connections. But it is key material in a stored,
  // shareable document, and if anyone ever strips the deck's guard the room is
  // joinable. fleet's bento_doc.py set removes it; Pages will not edit the
  // document block itself (its escaping rules are the helper's job), so say so.
  const docBlock = (html.match(/<script\b[^>]*application\/bento\+json[^>]*>([\s\S]*?)<\/script/i) || [])[1] || "";
  if (/"collab"\s*:\s*\{/.test(docBlock)) {
    warnings.push(
      finding(
        "bento_deck_collab_keys",
        "This deck's document carries a live-collaboration block (a room URL and a private key). Bento writes one into any file saved from its own UI. It is inert on this host — the deck's own CSP and the content host's both forbid connections — but it is key material in a stored, shareable document, and if the deck's guard is ever removed the room becomes joinable.",
        { fix: "Round-trip the file through fleet's bento_doc.py (`get`, then `set`) before deploying: `set` drops the block. Or accept the residue knowingly." }
      )
    );
  }
  return { deck: true, errors, warnings };
}

// ── R6: the script does not even parse ───────────────────────────────────────
// V8 classic scripts and Acorn modules, parse-only. Catches corruption that a
...
function checkSyntax(scripts) {
  ...
  for (const [i, script] of scripts.entries()) {
    ...
    try {
      if (script.module) {
        // Browser modules allow import/export, import.meta and top-level await.
        // Parse without resolving imports or evaluating any authored code; VM
        // module compilation requires a process-wide experimental Node flag.
        acorn.parse(script.src, { ecmaVersion: "latest", sourceType: "module" });
      } else {
        // eslint-disable-next-line no-new
        new vm.Script(script.src, { filename: `inline-script-${i}.js` });
      }
    } catch (err) {
      if (script.module && /Not enough stack space to parse input/.test(err.message || "")) {
        errors.push(finding("script_analysis_limit", `Inline <script> #${i + 1} exceeds the module parser's nesting limit; its syntax could not be verified.`, {
          script_index: i + 1,
          fix: "Reduce expression nesting and re-run preflight; this finding does not prove a browser syntax error.",
        }));
        continue;
      }
...
function analyze(html, { renderMode = "raw" } = {}) {
  ...
  const deck = checkBentoDeck(source, renderMode);
  const errors = [
    ...checkSyntax(scripts),
    ...handlers.errors,
  ...
  let warnings = [...handlers.warnings, ...checkOpaqueOrigin(scriptText), ...checkNavBlock(source)];
  // On a deck the one plaintext storage hit is fleet's guard script, which is
  // already wrapped in try/catch; the deck finding says what missing storage
  // means to a reader, so the generic one would only repeat it.
  if (deck.deck) warnings = warnings.filter((w) => w.code !== "opaque_origin_api");
