...
function checkBentoDeck(html, renderMode) {
  ...
  const warnings = [
    finding(
      "bento_deck",
      "This is a Bento deck. Its runtime is deflate-compressed, so the static checks here could not see the application and judged only its bootstrap. On the content host the deck opens in Bento's full editor. Nothing edited there is saved to Pages: Save writes or downloads a new .bento.html, Export PDF prints, and unsaved edits are gone on reload (the sandbox has no storage). Speaker view needs pop-ups, which the sandbox does not grant — Bento says so in its own words when asked. Embedded video and audio must be data: or blob: URLs.",
