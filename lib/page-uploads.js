...
// emits the base64, risks provider-side argument truncation mid-document).
function clampChunkBytes(raw, fallback, transportMax = TRANSPORT_MAX_CHUNK_BYTES) {
  const n = Number.parseInt(raw, 10);
  if (!Number.isInteger(n) || n < MIN_CHUNK_CEILING_BYTES || n > MAX_CHUNK_CEILING_BYTES || n > transportMax) {
    return fallback;
  }
  return n;
}
