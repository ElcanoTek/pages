...
const { MAX_UPLOAD_BYTES, DATA_LIMITS, assertStagedDataSize } = require("./data-limits");
...
const CHUNKING_HINT =
  ...
  "use a whole-file SHA-256 and exact byte range; the client encodes the file. Otherwise use a permitted " +
  "programmatic transport (create_upload_ticket for direct file HTTP). Keep accepted bytes and resume this upload.";
    ...
    next_step: complete
      ...
      : Number(row.bytes_received) === 0 && Number(row.total_bytes) > CHUNKING_ADVISORY_BYTES
        ? `This file is ${Number(row.total_bytes)} bytes. Continue this upload with append_page_upload ` +
          ...
          `Use the whole-file SHA-256, raw offset ${Number(row.bytes_received)}, at most ${MAX_CHUNK_BYTES} raw bytes, ` +
          `and sequence ${Number(row.next_sequence)}. Never print or generate base64 through model output. ` +
        ...
        : `Call append_page_upload with sequence ${row.next_sequence} for at most ${MAX_CHUNK_BYTES} raw bytes at offset ${Number(row.bytes_received)}. Use a workspace_file reference if advertised by your client, otherwise exact programmatic base64; do not generate it through model output.`,
  };
}
