...
const { MAX_UPLOAD_BYTES, MAX_REQUEST_BYTES, DATA_LIMITS, assertStagedDataSize } = require("./data-limits");
// When the calling MODEL writes the base64 itself, every byte is emitted token
// by token, so the ceiling is a direct multiplier on the cost and failure rate
// of a deploy. At the original 12 KiB a routine 65 KB dashboard took six perfect
...
// budget and cuts that to two calls. That is why it stays the default.
//
// A client that reads a workspace_file reference host-side never puts those
// bytes in model output, but each append is still one model tool call — and
// every call re-sends the whole conversation. At 48 KiB a 990 KB data file is 21
// appends. An operator whose clients all read files host-side can raise the
// ceiling (up to 1 MiB, which fits the default 2 MB request body as base64) and
// make that one append.
//
// This is a CEILING, not a required chunk size — sequences are size-independent,
// so a client that can only emit small arguments keeps working unchanged.
// Tunable for ops without a redeploy.
const MIN_CHUNK_CEILING_BYTES = 4 * 1024;
const MAX_CHUNK_CEILING_BYTES = 1024 * 1024;
// A chunk travels as base64 inside one JSON-RPC request, so a ceiling the
// request body cannot carry would be advertised and then refused with a 413
// before any tool runs. Reserve room for the envelope (id, method, upload_id,
// sequence, a client's _meta) around the base64.
const RPC_ENVELOPE_RESERVE_BYTES = 16 * 1024;
const TRANSPORT_MAX_CHUNK_BYTES = Math.floor((MAX_REQUEST_BYTES - RPC_ENVELOPE_RESERVE_BYTES) / 4) * 3;
// The smallest MAX_HTML_BYTES that can carry even the minimum chunk.
const MIN_TRANSPORT_REQUEST_BYTES = Math.ceil(MIN_CHUNK_CEILING_BYTES / 3) * 4 + RPC_ENVELOPE_RESERVE_BYTES;
// Resolved once at startup, and SAID when the setting is not what took effect:
// raising it is a production env change, and a silent fallback looks exactly
// like a rollout that never happened.
const CHUNK_CEILING = resolveChunkCeiling(process.env.PAGE_UPLOAD_MAX_CHUNK_BYTES, 48 * 1024);
if (CHUNK_CEILING.warning) console.warn(CHUNK_CEILING.warning);
const MAX_CHUNK_BYTES = CHUNK_CEILING.bytes;
...
const CHUNKING_HINT =
  "Do not hand-emit base64. If your client advertises workspace_file references on append_page_upload, " +
  "use a whole-file SHA-256 and exact byte ranges, as few appends as max_chunk_bytes allows; the client " +
  "encodes the file. Otherwise use a permitted programmatic transport (create_upload_ticket for direct file " +
  "HTTP). Keep accepted bytes and resume this upload.";
...
// trips, too large cannot fit one request body (and, for a client whose model
// emits the base64, risks provider-side argument truncation mid-document). A
// value outside them falls back to the default, and the default is held to the
// same transport bound, so the advertised ceiling is always one a request can
// carry — down to the 4 KiB floor, below which no MAX_HTML_BYTES is sensible.
// Returns { bytes, warning }; warning is null when nothing needs saying.
function resolveChunkCeiling(raw, fallback, transportMax = TRANSPORT_MAX_CHUNK_BYTES) {
  const usable = Math.max(MIN_CHUNK_CEILING_BYTES, Math.min(fallback, transportMax));
  const cramped = transportMax < MIN_CHUNK_CEILING_BYTES
    ? ` Even that does not fit one request; raise MAX_HTML_BYTES to at least ${MIN_TRANSPORT_REQUEST_BYTES} bytes.`
    : "";
  const configured = raw === undefined || raw === null ? "" : String(raw).trim();
  if (!configured) {
    if (usable === fallback) return { bytes: usable, warning: null };
    return {
      bytes: usable,
      warning:
        `WARNING: MAX_HTML_BYTES (${MAX_REQUEST_BYTES} bytes per request) cannot carry the default ${fallback}-byte ` +
        `upload chunk as base64; staged-upload chunks are limited to ${usable} bytes.${cramped}`,
    };
  }
  // The whole value must be decimal digits: parseInt would read "1048576oops"
  // or "1048576.5" as 1 MiB and raise the ceiling without the warning.
  const n = /^[0-9]+$/.test(configured) ? Number(configured) : NaN;
  let reason = null;
  if (!Number.isInteger(n)) reason = "it is not a whole number of bytes";
  else if (n < MIN_CHUNK_CEILING_BYTES) reason = `it is below the ${MIN_CHUNK_CEILING_BYTES}-byte minimum`;
  else if (n > MAX_CHUNK_CEILING_BYTES) reason = `it is above the ${MAX_CHUNK_CEILING_BYTES}-byte maximum`;
  else if (n > transportMax) {
    reason = `its base64 does not fit one ${MAX_REQUEST_BYTES}-byte request (MAX_HTML_BYTES carries at most ` +
      `${Math.max(transportMax, 0)} raw bytes per chunk)`;
  }
  if (!reason) return { bytes: n, warning: null };
  return {
    bytes: usable,
    warning: `WARNING: PAGE_UPLOAD_MAX_CHUNK_BYTES="${configured}" is not used because ${reason}; ` +
      `staged-upload chunks are limited to ${usable} bytes.${cramped}`,
  };
}

function clampChunkBytes(raw, fallback, transportMax = TRANSPORT_MAX_CHUNK_BYTES) {
  return resolveChunkCeiling(raw, fallback, transportMax).bytes;
}
...
function state(row) {
  ...
  const remaining = Number(row.total_bytes) - Number(row.bytes_received);
  // Say the exact count: "at most N bytes" alone read as permission to send many
  // small chunks, and each append is a full model round trip.
  const appendsLeft = Math.ceil(remaining / MAX_CHUNK_BYTES);
  ...
  return {
    ...
    ...(kind === "data" ? { data_limits: DATA_LIMITS } : {}),
    complete,
    ...
    next_step: complete
      ...
      : Number(row.bytes_received) === 0 && Number(row.total_bytes) > CHUNKING_ADVISORY_BYTES
        ? `This file is ${Number(row.total_bytes)} bytes. Continue this upload with append_page_upload ` +
          `using workspace_file references if your client advertises them, or exact programmatic base64. ` +
          `Use the whole-file SHA-256, raw offset ${Number(row.bytes_received)} and sequence ${Number(row.next_sequence)}, ` +
          `in as few appends as max_chunk_bytes (${MAX_CHUNK_BYTES}) allows: ${appendsLeft} ` +
          `append${appendsLeft === 1 ? "" : "s"} for this file. Never print or generate base64 through model output. ` +
          `Do not cancel a usable upload to change transport. For a future upload without this capability, ` +
          `create_upload_ticket supports direct file HTTP when permitted.`
        : `Call append_page_upload with sequence ${row.next_sequence} for the next ${Math.min(remaining, MAX_CHUNK_BYTES)} raw bytes at offset ${Number(row.bytes_received)} ` +
          `(as few appends as max_chunk_bytes ${MAX_CHUNK_BYTES} allows: ${appendsLeft} left). Use a workspace_file reference if advertised by your client, otherwise exact programmatic base64; do not generate it through model output.`,
  };
}
...
function attemptAlarm(counts) {
  ...
  return (
    ...
    `before the file lands. Resume a usable upload with file-reference appends or another permitted ` +
    `programmatic transport. create_upload_ticket is available for direct file HTTP, but do not restart ` +
    `an existing valid transfer or hand-emit base64.`
  );
}
...
async function start({ slug, template, kind, totalBytes, contentSha256, dataSize }, ctx) {
  const target = normalizeTarget({ slug, template, kind });
  assertDeclaredContent(totalBytes, contentSha256);
  assertStagedDataSize(target.kind, dataSize);
...
async function createTicket({ slug, template, kind, totalBytes, contentSha256, dataSize }, ctx) {
  const target = normalizeTarget({ slug, template, kind });
  assertDeclaredContent(totalBytes, contentSha256);
  assertStagedDataSize(target.kind, dataSize);
  const tokenId = actorTokenId(ctx);
...
async function deploy(uploadId, ctx, commitKey, deployVerified, { expectKind = "page", replayCommitted } = {}) {
  ...
  return db.withTransaction(async (client) => {
    ...
    if (upload.commit_result) {
      ...
      // The saved receipt is immutable. A consumer may refresh read-only
      // guidance for this reply, but must never run the mutation again.
      return replayCommitted
        ? canonicalJson(await replayCommitted(client, upload.commit_result))
        : upload.commit_result;
    }
    const html = await readVerified(client, upload);
...
module.exports = {
  ...
  MIN_CHUNK_CEILING_BYTES,
  MAX_CHUNK_CEILING_BYTES,
  TRANSPORT_MAX_CHUNK_BYTES,
  MIN_TRANSPORT_REQUEST_BYTES,
  clampChunkBytes,
  resolveChunkCeiling,
  MAX_ACTIVE_UPLOADS,
