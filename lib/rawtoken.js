...
//
//   edit     — open ONE deck page in its own editor at /raw with a save channel
//              back to Pages, and authorise the saves. Minted only by the admin
//              (staff SSO) for a page whose current version is a Bento deck;
//              bound to the page, not a version, because every save makes a new
//              one; carries the staff actor so the audit row names a person. It
//              buys the right to add DRAFT versions to that page for its TTL and
//              nothing else — no publish, no read of anything the page does not
//              already show. ("edit" was once listed here for an editor that was
//              never built, and removed as an unused door; this is that door,
//              built, and consumed by exactly two routes in server.js.)
const PURPOSES = new Set(["view", "template", "session", "edit"]);
...
// mint a token. claims: { pageId, versionId, purpose, renderMode, sid?, actor? }.
// `actor` is required for "edit" (the audit row of every save names it) and
// refused for every other purpose, so a render credential can never carry an
// identity it has no use for.
function mint({ pageId, versionId, purpose, renderMode, sid, actor }, ttlSeconds = 300) {
  if (!SECRET) throw new Error("RAW_TOKEN_SECRET is not set");
  if (!PURPOSES.has(purpose)) throw new Error(`bad purpose: ${purpose}`);
  if (purpose === "edit" && (typeof actor !== "string" || !actor)) throw new Error("an edit token must name its actor");
  if (purpose !== "edit" && actor !== undefined) throw new Error(`a ${purpose} token carries no actor`);
  const payload = {
    pid: pageId,
    vid: versionId,
    purpose,
