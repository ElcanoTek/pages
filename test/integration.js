...
//
//   VID_NORTHWIND = id of a themed, live version of page 'northwind'
...
const VID_NORTHWIND = Number(process.env.VID_NORTHWIND);
...
(async () => {
  ...
  try {
    ...
    const tok = rawtoken.mint({ pageId: 0, versionId: VID_NORTHWIND, purpose: "view", renderMode: "themed" });
    const ok = await get(`/raw/northwind?t=${encodeURIComponent(tok)}`, CONTENT);
    ...
    // 2. No token → 403.
    const noTok = await get(`/raw/northwind`, CONTENT);
    ...
    const wrongSlug = await get(`/raw/notnorthwind?t=${encodeURIComponent(tok)}`, CONTENT);
