...
(async () => {
  ...
  try {
    ...
    assert.equal(indexedMetadata.data_template_sha256, parsedMetadata.template_sha256);
    assert.equal(indexedMetadata.refreshed_at.toISOString(), "2026-08-01T01:00:00.000Z");
    assert.equal(indexedMetadata.source_as_of.toISOString(), "2026-08-01T00:00:00.000Z");
    assert.equal(indexedMetadata.template_version_id, null);
    assert.equal(metadataDraft.json.published, false);
    console.log("✓ admin source drafts index their embedded managed data without acquiring a template binding");
